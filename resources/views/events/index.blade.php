  ...
  <div class="container">
    <div class="event-filter-bar fade-in-up">
      ...
      <div class="event-view-toggle">
        <button class="view-btn active" id="viewGrid" title="Grid view" aria-label="Tampilan grid">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="1" y="1" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.5"/><rect x="9" y="1" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.5"/><rect x="1" y="9" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.5"/><rect x="9" y="9" width="6" height="6" rx="1" stroke="currentColor" stroke-width="1.5"/></svg>
        </button>
        <button class="view-btn" id="viewList" title="List view" aria-label="Tampilan list">
  ...
  <div class="container">
    <div class="event-countdown-banner fade-in-up">
      ...
      <div class="countdown-info">
        <div class="countdown-title">Kompetisi Selam Bali Open 2026</div>
        <div class="countdown-date">📅 Sabtu, 12 Juli 2026 · Pantai Sanur, Denpasar</div>
      </div>
      ...
      <a href="{{ url('/events/1') }}" class="btn-primary btn-sm"><span>Daftar Sekarang</span></a>
    </div>
  </div>
</section>
...
<section class="section events-section" style="padding-top:2.5rem;">
  <div class="container">
    ...
    <div class="events-card-grid" id="eventsGrid">
      ...
      @php
        $events = [
          ['day'=>'12','month'=>'JUL','type'=>'Kompetisi','cat'=>'kompetisi','icon'=>'🏆',
           'title'=>'Kompetisi Selam Bali Open 2026',
           'desc'=>'Turnamen selam terbuka tingkat nasional dengan berbagai kategori: freediving, scuba, dan underwater photography.',
           'loc'=>'Pantai Sanur, Denpasar','time'=>'07.00 – 17.00 WITA','slots'=>'120 / 200','status'=>'open'],
          ['day'=>'19','month'=>'JUL','type'=>'Pelatihan','cat'=>'pelatihan','icon'=>'🤿',
           'title'=>'Pelatihan Selam Dasar Bersertifikat',
           'desc'=>'Kursus selam dasar selama 3 hari dengan instruktur bersertifikat CMAS & SSI. Termasuk teori dan praktek laut.',
           'loc'=>'Kolam Renang Tirta Yasa, Denpasar','time'=>'08.00 – 16.00 WITA','slots'=>'18 / 25','status'=>'open'],
          ['day'=>'26','month'=>'JUL','type'=>'Sosial','cat'=>'sosial','icon'=>'🌿',
           'title'=>'Bersih Laut Bersama — Nusa Penida',
           'desc'=>'Aksi nyata menjaga kebersihan perairan Nusa Penida bersama ratusan penyelam sukarela dan komunitas lokal.',
           'loc'=>'Crystal Bay, Nusa Penida','time'=>'06.00 – 13.00 WITA','slots'=>'85 / 150','status'=>'open'],
          ['day'=>'02','month'=>'AGS','type'=>'Seminar','cat'=>'seminar','icon'=>'🎓',
           'title'=>'Seminar Nasional: Masa Depan Selam Indonesia',
           'desc'=>'Diskusi panel dengan pakar kelautan, pelatih nasional, dan pejabat pemerintah tentang pengembangan olahraga selam.',
           'loc'=>'Hotel Grand Inna Bali Beach','time'=>'09.00 – 17.00 WITA','slots'=>'67 / 100','status'=>'open'],
          ['day'=>'09','month'=>'AGS','type'=>'Kompetisi','cat'=>'kompetisi','icon'=>'🥇',
           'title'=>'Pra-PON Selam 2026 — Seleksi Bali',
           'desc'=>'Seleksi resmi atlet selam Bali untuk Pekan Olahraga Nasional 2026. Wajib diikuti atlet binaan POSSI Bali.',
           'loc'=>'Pantai Kuta, Badung','time'=>'06.30 – 15.00 WITA','slots'=>'40 / 40','status'=>'penuh'],
          ['day'=>'16','month'=>'AGS','type'=>'Pelatihan','cat'=>'pelatihan','icon'=>'📸',
           'title'=>'Workshop Fotografi Bawah Laut',
           'desc'=>'Pelajari teknik underwater photography dari fotografer profesional. Sertifikat peserta dan galeri pameran foto.',
           'loc'=>'Tulamben, Karangasem','time'=>'07.00 – 16.00 WITA','slots'=>'12 / 15','status'=>'hampir penuh'],
        ];
      @endphp

      @foreach($events as $i => $ev)
      <div class="event-card-v2 fade-in-up delay-{{ ($i % 3) + 1 }}" data-category="{{ $ev['cat'] }}">
        <div class="event-card-v2-header">
          <div class="event-date-badge">
            <div class="event-date-day">{{ $ev['day'] }}</div>
            <div class="event-date-month">{{ $ev['month'] }}</div>
          </div>
          <div class="event-type-badge event-type-{{ $ev['cat'] }}">{{ $ev['type'] }}</div>
          <div class="event-icon-wrap">{{ $ev['icon'] }}</div>
        </div>

        <div class="event-card-v2-body">
          <h3 class="event-card-v2-title">{{ $ev['title'] }}</h3>
          <p class="event-card-v2-desc">{{ $ev['desc'] }}</p>
          ...
          <!-- Slot progress bar -->
          @php
            [$used, $total] = explode(' / ', $ev['slots']);
            $pct = round(($used / $total) * 100);
          @endphp
          <div class="event-slots">
            <div class="event-slots-label">
              <span>Pendaftar</span>
              <span class="event-slots-num">{{ $ev['slots'] }}</span>
            </div>
            <div class="event-slots-bar">
              <div class="event-slots-fill event-slots-{{ $ev['status'] === 'penuh' ? 'full' : ($ev['status'] === 'hampir penuh' ? 'almost' : 'open') }}"
                   style="width:{{ $pct }}%"></div>
            </div>
          </div>
        </div>
        ...
        <div class="event-card-v2-footer">
          @if($ev['status'] === 'penuh')
            ...
            <button class="btn-outline btn-sm" disabled>Daftar Tunggu</button>
          @elseif($ev['status'] === 'hampir penuh')
            <span class="event-status-badge status-hampir">Hampir Penuh</span>
            <a href="{{ url('/events/'.($i+1)) }}" class="btn-primary btn-sm"><span>Daftar Sekarang</span></a>
          @else
    ...
    <div class="empty-state" id="eventsEmpty" style="display:none;">
      ...
      <div class="empty-state-desc">Coba pilih kategori lain untuk melihat event tersedia.</div>
    </div>
...
.view-btn {
  ...
  color: rgba(247,251,252,.5); cursor:pointer; display:flex;
  align-items:center; justify-content:center; transition: all var(--transition);
}
.view-btn.active, .view-btn:hover {
  background: rgba(26,179,216,.12); border-color: var(--ocean-bright);
...
.countdown-badge {
  ...
  background: rgba(224,92,58,.2); border:1px solid rgba(224,92,58,.4);
  font-size:.72rem; font-weight:700; color: var(--ocean-coral);
...
document.addEventListener('DOMContentLoaded', () => {
  /* ── COUNTDOWN ── */
  const target = new Date('2026-07-12T07:00:00+08:00');
